...
def run(
    ...
    ltc_modal = NCPModel(model_id = MODEL_PATH, DEVICE = DEVICE).to(DEVICE)
    ...
    for i in range(0, int(ARGS.duration_sec*env.PYB_FREQ)):
        ...
        cv2.imshow(" dep", dep )
        cv2.imshow("seg ", seg)
        inputs_image = processor.image(images=rgb, return_tensors="pt").to(DEVICE, torch.float16)

        with torch.no_grad():
            liquid_output = ltc_modal(input_ids=inputs_text["input_ids"],

    ...
    try:
        with open(file_path, 'r') as file:
            for line in file:
                ...
                roll, thrust, pitch, yaw = map(float, values)
...
def run(
    ...
    #### Get the Roll, pitch, yaw and thrust from the txt file ######################
    file_path = 'output.txt'
    movements = read_movements_from_file(file_path)
    #for movement in movements:
    #    print(movement)


    #### Initialize a circular trajectory ######################
    print("movements lenght",len(movements))
    NUM_WP = len(movements)    #simulation will run for the length of movements
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        for j in range(num_drones):
            action[j, :], _, _ = ctrl[j].computeControlFromState(control_timestep=env.CTRL_TIMESTEP,
                                                                    ...
                                                                    target_rpy=movements[wp_counters[j]]
                                                                    )

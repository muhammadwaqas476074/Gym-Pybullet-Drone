...
def read_movements_from_file(file_path):
    """
    Reads thrust, pitch, yaw, and roll movements from a text file.
    
    Parameters:
    - file_path (str): Path to the text file containing the movements.
    
    Returns:
    - movements (list of dict): List of dictionaries containing thrust, pitch, yaw, and roll for each entry.
    """
    movements = []
    ...
    try:
        with open(file_path, 'r') as file:
            for line in file:
                # Assuming the format of each line in the file is: thrust, pitch, yaw, roll
                ...
                #thrust, yaw, roll, pitch = map(float, values) 
                # if these values were + then the movement should be
                #left, forward, right, UP

                roll, pitch, yaw, thrust = map(float, values)
                
                roll, pitch, yaw, thrust = roll, pitch, yaw, thrust
...
def run(
    ...
    for i in range(0, len(movements)):
        ...
        for j in range(num_drones):
            if wp_counters[j] < len(movements):
                target_rpy = [movements[wp_counters[j]]['roll'],
                              movements[wp_counters[j]]['pitch'],
                              movements[wp_counters[j]]['yaw']]
                target_thrust = movements[wp_counters[j]]['thrust']
                print(target_rpy, " rpy from full")

                #if target_thrust != 0:
                #    INIT_XYZS[j, :] = obs[j][0:3]  
        ...
        for j in range(num_drones):
            wp_counters[j] = wp_counters[j] + 1 if wp_counters[j] < (len(movements) - 1) else 0
        ...
        camera_image = get_drone_camera_image(env.DRONE_IDS[0])
        cv2.imshow("Drone Camera", camera_image)
